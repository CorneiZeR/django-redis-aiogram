...
A growing list means the consumer is not running — see above. The list is
durable, so everything is delivered once it comes back.
...
one `getUpdates` consumer per bot. Delivery itself takes each message once.
...
```python
LOGGING = {'loggers': {'django_redis_aiogram': {'level': 'DEBUG'}}}
```
