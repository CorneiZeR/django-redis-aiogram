...
If you needed `'ALLOW_PICKLE': True` for the upgrade window, remove it as soon
as the old messages are gone — it is the code-execution door.
...
Set `'DELIVERY': 'keyspace'` to keep the old delivery mechanism.
